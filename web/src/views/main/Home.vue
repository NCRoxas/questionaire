<template>
	<div class="container mt-4 mb-4">
		<div class="row">
			<h1 class="text-center text-success">Ihre Pakete</h1>
			<div class="col-md-3 p-3" v-for="(element, id) in packs" :key="id">
				<div class="card">
					<Delete v-if="editmode" :item="element" @remove-item="remove" />

					<router-link :to="{ name: 'category', params: { pack: element.slug } }" :aria-label="element.slug">
						<img
							v-if="!editmode"
			...
			<Create v-if="editmode" label="Paket" @add-item="add" />
		</div>
	</div>
</template>
...
const { packs, editmode } = mapGetters();
const { getPacks, newPack, updatePack, deletePack } = mapActions();
const cacheKey = ref(0);

const add = (data) => {
	newPack(data).then(() => getPacks());
};
const remove = (data) => {
